...
Out-user limits that depend on state accumulated across requests (`max_concurrent_queries`, `max_queue_size`, `requests_per_minute`, and the rate component of `request_packet_size_tokens_*`) are not enforced for wildcarded users today, because each authenticated request resolves to a fresh clone of the cluster user. The per-request `request_packet_size_tokens_burst` is enforced — a single request whose body exceeds the burst is rejected. In-user limits work normally; all requests resolved through the same wildcarded in-user pattern share that in-user's counters.
